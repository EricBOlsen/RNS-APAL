...
SPMF::SPMF(__int64 x) : SPPM(x)
{
	...
	NumFractDigits = SPMF_FRACTION_DIGS;
	NormalFractDigits = NumFractDigits;

	if(NumFractDigits > NUM_PPM_DIGS) {		// store the number of fractional digits
...
//	int index = this->NumDigits - 1;			// start with largest digit for optimization
	...
	if(index < SPMF_FRACTION_DIGS) {		// if digit conversion < NumFracDigs

		index2 = index;
		int primemod;
		while((SPMF_FRACTION_DIGS - index2) > 1) {      // do all remaining digits except the last one before the normal fraction point.
...
//	int index = this->NumDigits - 1;			// start with largest digit for optimization
	...
	if(index < SPMF_FRACTION_DIGS) {		// if digit conversion < NumFracDigs
		...
		index2 = index;
		int primemod;
		while((SPMF_FRACTION_DIGS - index2) > limit) {      // do all remaining digits except the last one before the normal fraction point.
